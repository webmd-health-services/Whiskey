...
function ConvertTo-WhiskeySemanticVersion
{
    ...
    .EXAMPLE
    '1.2.3+build.info' | ConvertTo-WhiskeySemanticVersion -PreserveBuildMetadata
    ...
    param(
        ...
        [Switch]
        # Use the build metadata from the original object, even if it doesn't exist. This stops `ConvertTo-WhiskeySemanticVersion` from setting the build metadata based on the current build.
    ...
    process
    {
        ...
        {
            $patch = $buildID = Get-WhiskeyBuildID
            $branch = Get-WhiskeyBranch
            $branch = $branch -replace '[^A-Za-z0-9-]','-'

            if( -not (Test-Path -Path 'env:GIT_COMMIT') )
            {
                throw ('Environment variable GIT_COMMIT does not exist. Are you sure you''re running under a build server? If you see this message while running tests, you most likely need to mock the `ConvertTo-WhiskeySemanticVersion` function.')
            }
