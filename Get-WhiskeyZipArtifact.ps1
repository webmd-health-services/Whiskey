...
{
    $msg = 'Multiple Jobs found for the latest build. Please specify the -JobName parameter to select which job you ' +
...
$zipFileName = $artifacts | Where-Object 'fileName' -Like 'Whiskey*.zip' | Select-Object -ExpandProperty 'fileName'
if (-not $zipFileName)
{
    $msg = 'Unable to download Whiskey ZIP file artifact because it does not exist.'
...
Rename-Item -Path 'Whiskey' -NewName 'Whiskey.orig'
