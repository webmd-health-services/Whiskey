...
function Assert-WhiskeyNodeModulePath
{
    ...
    {
        Write-Error -Message ('Node module ''{0}'' does not exist at ''{1}''. Whiskey or NPM maybe failed to install this module correctly. Clean your build then re-run your build normally. If the problem persists, it might be a task authoring error. Please see the `about_Whiskey_Writing_Tasks` help topic for more information.' -f ($Path | Split-Path -Leaf),$Path)        
        return
    }
