...
function Test-WhiskeyTaskSkip
{
    ...
    elseif( $Properties['OnlyIfBuildStatusIs'] )
    {
        ...
        {
            Stop-WhiskeyTask -TaskContext $Context -PropertyName 'OnlyIfBuildStatusIs' -Message ('invalid value: ''{0}''. Valid values are ''{1}''.' -f $Properties['OnlyIfBuildStatusIs'],([enum]::GetValues([Whiskey.BuildStatus]) -join ''', '''))
            return
        }
