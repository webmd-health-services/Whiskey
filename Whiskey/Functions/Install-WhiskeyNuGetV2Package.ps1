...
function Install-WhiskeyNuGetV2Package
{
    param(
        [Parameter(Mandatory)]
        [String] $Name,
    ...
    {
        New-Item -Path $pkgCachePath -ItemType 'Directory' | Out-Null
    }
    ...
    if( -not (Test-Path -Path $nupkgPath) )
    {
        ...
        for( $idx = 0; $idx -lt $numTries; ++$idx )
        {
            ...
            $pkg |
                Save-Package -Path $pkgCachePath -ErrorAction SilentlyContinue -ErrorVariable 'pkgMgmtErrors' -Force |
                Out-Null
    ...
    foreach( $pkgInfo in (Get-ChildItem -Path $pkgCachePath -Filter '*.nupkg') )
    {
