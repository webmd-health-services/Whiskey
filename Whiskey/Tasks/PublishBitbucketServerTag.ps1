...
function Publish-WhiskeyBBServerTag
{
    ...
    else
    {
        $msg = 'Unable to determine the repository where we should create the tag. Either create a `GIT_URL` ' +
               'environment variable that is the URI used to clone your repository, or add your repository''s ' +
        ...
        "
        Stop-WhiskeyTask -TaskContext $TaskContext -PropertyDescription '' -Message
        return
    }
